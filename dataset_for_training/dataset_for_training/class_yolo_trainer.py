...
"""
    ...
    def training_YOLO_model(self):
        ...
        try:
            results = model.train(data = "dataset/dataset_YOLO/dataset.yaml", epochs=10, imgsz=640, device="cuda")
        except AssertionError as e:
            e("CUDA is not avaible, switching to cpu")
            results = model.train(data = "dataset/dataset_YOLO/dataset.yaml", epochs=10, imgsz=640, device="cpu")
    ...
    def predict_YOLO_model(self, train="train"):
        ...
        model.predict(self.object_to_predict, save=True, conf=self.predict_confidence, device="cuda")
